...
def get_delivered_or_admin_messages() -> List[Message]:
    return list(Message.objects.filter(
        Q(is_delivered=1) | (Q(user__first_name__startswith="admin"))))


def get_count_messages_sent_by_first_name(first_name: str) -> int:
    return Message.objects.filter(user__first_name__startswith=first_name)\
        .count()
...
def get_last_5_messages_dicts() -> List[dict]:
    return [
        ...
        for mes in Message.objects.select_related("user").order_by("-sent")[:5]
    ]
