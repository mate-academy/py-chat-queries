...
def get_actual_chats() -> list[Chat]:
    return list(
        Chat.objects.filter(message__sent__gt=datetime.date(2020, 12, 31))
    )


def get_messages_contain_authors_first_name() -> list[Message]:
    return list(
        Message.objects.filter(text__contains=F("user__first_name")).distinct()
    )
...
def get_last_5_messages_dicts() -> list[dict]:
    return [
        ...
        for message
        in Message.objects.select_related("user").order_by("-sent")[:5]
    ]
