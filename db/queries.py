...
def get_users_who_sent_messages_starts_with_m_or_a() -> list[User]:
    return list(User.objects.annotate(Count("message__user")).filter(
        Q(message__text__istartswith="a")
        | Q(message__text__istartswith="m"),
        message__user__count__gte=1
    ))
...
def get_last_5_messages_dicts() -> list[dict]:
    return [
        {"from": name, "text": text}
        for name, text in Message.objects.select_related("user").order_by(
            "-sent"
        )[:5].values_list("user__username", "text")
    ]
