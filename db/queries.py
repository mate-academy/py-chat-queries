...
def get_users_who_sent_messages_in_2015() -> list[str]:
    return list(Message.objects.filter(
        sent__year=2015).values_list("user__first_name",
                                     "user__last_name"))
...
def get_delivered_or_admin_messages() -> list[Message]:
    return list(Message.objects.filter(
        Q(is_delivered=1) | Q(user__username__istartswith="admin")))


def get_count_messages_sent_by_first_name(first_name: str) -> int:
    result_dict = Message.objects.filter(
        user__first_name=first_name).aggregate(count=Count("user"))
...
def get_last_5_messages_dicts() -> list[dict]:
    last_message = list(Message.objects.all().select_related(
    ...
    return [
        {
            "id": chat.id,
            "title": chat.title,
            "users": [usr["username"] for usr in chat.users.values("username")]
        }
        for chat in chats
    ]
