...
def get_count_messages_sent_by_first_name(first_name: str) -> int:
    queryset = User.objects.values("first_name")\
        .annotate(Count("message")).filter(first_name=first_name)
    return queryset[0]["message__count"]
...
def get_chat_dicts() -> list[dict]:
    ...
    for chat in Chat.objects.all().prefetch_related("users"):
        chat_users = [user.username for user in chat.users.all()]
        result.append({
            "id": chat.id,
            "title": chat.title,
            "users": chat_users
