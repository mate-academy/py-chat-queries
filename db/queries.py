...
def get_count_messages_sent_by_first_name(first_name: str) -> int:
    messages = Message.objects.filter(user__first_name=first_name)\
        .aggregate(count_messages=Count("chat"))
    return messages["count_messages"]


def get_top_users_by_number_of_the_messages() -> list[User]:
    return list(User.objects.annotate(
        num_messages=Count("message__chat")).order_by("-num_messages")[:3])


def get_last_5_messages_dicts() -> list[dict]:
    messages = list(Message.objects.order_by("-sent")[:5].select_related())
    return [
        {
...
def get_chat_dicts() -> list[dict]:
    return [
        ...
        for chat in Chat.objects.prefetch_related()
    ]
