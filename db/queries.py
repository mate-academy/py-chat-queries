...
def get_untitled_chats() -> list[Chat]:
    return Chat.objects.all().filter(title__startswith="Untitled")


def get_users_who_sent_messages_in_2015() -> list[str]:
    return list(Message.objects.all().filter(sent__year=2015)
                .values_list("user__first_name", "user__last_name"))


def get_actual_chats() -> list[Chat]:
    return Chat.objects.all() \
        .filter(message__sent__gte="2020-01-01") \
        .distinct()
...
def get_top_users_by_number_of_the_messages() -> list[User]:
    return list(User.objects.annotate(
        num_messages=Count("message__user__id")
