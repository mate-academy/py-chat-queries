...
def get_users_who_sent_messages_in_2015() -> list[str]:
    return list(Message.objects.filter(sent__date__year='2015').
                values_list('user__first_name', 'user__last_name'))


def get_actual_chats() -> list[Chat]:
    return list(Chat.objects.filter(message__sent__date__year__gt='2020'))
...
def get_last_5_messages_dicts() -> list[dict]:
    all_messages = Message.objects.select_related("user").order_by("-sent")[:5]
    return [
...
def get_chat_dicts() -> list[dict]:
    all_chats = Chat.objects.all().prefetch_related("users")
    return [
        {
            "id": chat.id,
            "title": chat.title,
