...
def get_actual_chats() -> list[Chat]:
    return list(
        Chat.objects.distinct().filter(message__sent__year__gt=2020)
    )
...
def get_users_who_sent_messages_starts_with_m_or_a() -> list[User]:
    return list(
        User.objects.annotate(
            count_messages=Count("message")
        ).filter(
            Q(message__text__startswith="a") | Q(
                message__text__startswith="m"
            ),
            count_messages__gte=1
        )
    )
...
def get_last_5_messages_dicts() -> list[dict]:
    msg_query = Message.objects.order_by("-sent").select_related("user")[:5]
    return [
...
def get_chat_dicts() -> list[dict]:
    return [
        {"id": chat.id, "title": chat.title, "users": [
            user for user in chat.users.values_list("username", flat=True)
        ]}
        for chat in Chat.objects.prefetch_related("users").all()
    ]
