...
def get_actual_chats() -> list[Chat]:
    message_queryset = Message.objects.filter(sent__year__gt="2020")
    return [message.chat for message in message_queryset]
...
def get_users_who_sent_messages_starts_with_m_or_a() -> list[User]:
    ...
    return [message.user for message in message_queryset]
...
def get_chat_dicts() -> list[dict]:
    chat_queryset = Chat.objects.all().prefetch_related("users")
    return [
        {"id": chat.id,
         "title": chat.title,
