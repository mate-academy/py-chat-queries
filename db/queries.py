...
def get_users_who_sent_messages_starts_with_m_or_a() -> list[User]:
    return list(User.objects.filter(Q(message__text__istartswith="M")
                                    | Q(message__text__istartswith="A")))


def get_delivered_or_admin_messages() -> list[Message]:
    return list(Message.objects.filter(Q(user__username__startswith="admin")
                                       | Q(is_delivered=True)))
...
def get_top_users_by_number_of_the_messages() -> list[User]:
    return User.objects.annotate(
        num_messages=Count("message__sent")).order_by("-num_messages")[:3]


def get_last_5_messages_dicts() -> list[dict]:
    related = Message.objects.select_related("user").order_by("-sent")[:5]
    return list(
        {
            "from": mes.user.username, "text": mes.text
        }
        for mes in related
    )
...
def get_chat_dicts() -> list[dict]:
    related = Chat.objects.prefetch_related("users")
    return list(
        {
            "id": chat.id,
            "title": chat.title,
