...
def get_top_users_by_number_of_the_messages() -> list[User]:
    return list(User.objects.annotate(
        num_messages=Count(F("message"))
    ).order_by("-num_messages")[:3])


def get_last_5_messages_dicts() -> list[dict]:
    queryset = Message.objects.order_by("-sent").values_list(
        "user__username",
        "text"
    )[:5]
    return [{"from": item[0], "text": item[1]}for item in queryset]


def get_chat_dicts() -> list[dict]:
    return [{"id": chat.id, "title": chat.title, "users": [user.username
