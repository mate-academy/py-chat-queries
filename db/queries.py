...
def get_actual_chats() -> list[Chat]:
    return list(Chat.objects.filter(message__sent__year__gte=2022).distinct())
...
def get_users_who_sent_messages_starts_with_m_or_a() -> list[User]:
    return list(User.objects.filter(
        Q(message__text__istartswith="m") | Q
        (message__text__istartswith="a")))


def get_delivered_or_admin_messages() -> list[Message]:
    return list(Message.objects.filter(Q(is_delivered=True) | Q
...
def get_last_5_messages_dicts() -> list[dict]:
    return [
        ...
        for message in
        Message.objects.order_by("-sent").select_related("user")[:5]]
...
def get_chat_dicts() -> list[dict]:
    return [
        ...
        Chat.objects.all().prefetch_related("users")
    ]
