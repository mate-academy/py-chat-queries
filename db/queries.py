...
def get_untitled_chats() -> list[Chat]:
    return list(Chat.objects.filter(title__startswich="Untitled"))


def get_users_who_sent_messages_in_2015() -> list[str]:
    return list(User.objects.filter(message__sent__year=2015).value_list(
        "first_name",
        "last_name",
    ))
...
def get_users_who_sent_messages_starts_with_m_or_a() -> list[User]:
    return list(User.objects.filter(Q(
        message__sent__istartswith="a") | Q(
        message__sent__istartswith="m"
    )))
...
def get_delivered_or_admin_messages() -> list[Message]:
    return list(Message.objects.filter(Q(
        is_delivered="1") | Q(
        user__username__contains="admin"
    )))
...
def get_chat_dicts() -> list[dict]:
    return list(
        {"id": chat.id,
         "title": chat.title,
         "users": [user.user_name for user in chat.users.all()]}
